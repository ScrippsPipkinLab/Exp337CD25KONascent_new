    ...
    for root, dirs, files in os.walk(wkDir):
        for file in files:
            if not file.startswith("._"):
                if file.endswith("GN.csv"):
                    filelist.append(os.path.join(root, file))
    fnames = [x.split("/")[-1].replace(".csv", "").replace("_addGN", "") for x in filelist]
    print(fnames)
    
    with open(outName, "w") as fout:
        wfout = csv.writer(fout, delimiter=",")
        header = ["transcript", "gene_name"] + fnames
...
wk_dir = "/Volumes/Yolanda/Exp337CD25KONascent/3_DE-seq/1_compiled"
os.chdir(wk_dir)

DEseq2out_dir = "/Volumes/Yolanda/Exp337CD25KONascent/3_DE-seq/0.1_original_GN/dupr"
CompileTables(DEseq2out_dir, "dupr_DEseq_pval.csv", 5)
CompileTables(DEseq2out_dir, "dupr_DEseq_padj.csv", 6)
CompileTables(DEseq2out_dir, "dupr_DEseq_log2fc.csv", 2)
CompileTables(DEseq2out_dir, "dupr_DEseq_baseMean.csv", 1)

DEseq2out_dir = "/Volumes/Yolanda/Exp337CD25KONascent/3_DE-seq/0.1_original_GN/nondupr"
CompileTables(DEseq2out_dir, "nondupr_DEseq_pval.csv", 5)
CompileTables(DEseq2out_dir, "nondupr_DEseq_padj.csv", 6)
CompileTables(DEseq2out_dir, "nondupr_DEseq_log2fc.csv", 2)
CompileTables(DEseq2out_dir, "nondupr_DEseq_baseMean.csv", 1)
